...
import parse, {line} from './parse.js'

interface consts {
  [key: string]: number
}

function resolveLine(line: line, consts: consts, places: consts, last: boolean) {
  const {inst, args} = line
  if (!inst) return []

  const vals: (string | number)[] = args.map(arg => {
    switch (arg.type) {
...
function assertDefine(dest: consts, prop: string, val: number) {
  assertLine(typeof dest[prop] == 'undefined', 'Already defined')
  dest[prop] = val
}

export default function compile(sourceCode: string): line[] {
  const source: line[] = parse(sourceCode)
