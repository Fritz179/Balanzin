...
window.addEventListener('load', () => {
  ...
  lowButton.onclick = () => send(parse(request.lowBytes) as number[])
  highButton.onclick = () => send(parse(request.highBytes) as number[])
  ...
  function parse(requestType: request) {
    run([])
    try {
      ...
      // for run button
      if (requestType == request.nothing) return []
      if (requestType == request.program) return program
  ...
  runButton.onclick = () => {
    run(parse(request.program) as line[]);
  }
