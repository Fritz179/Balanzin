import { ShapeTypes, Rect, Circle, Line, Point } from '../math/Shape.js'
import ChildrenSet from './ChildrenBox.js'

abstract class BasicRenderer {
  abstract render(renderer: JABSRenderer): void
}

export default class JABSRenderer extends ChildrenSet<BasicRenderer> {
  canvas: HTMLCanvasElement
  #ctx: CanvasRenderingContext2D
  children = new Set<BasicRenderer>()

  constructor(master: any, canvas: HTMLCanvasElement) {
    super(master)
  ...
  shape(shape: ShapeTypes, fill?: string | false, stroke?: string, width?: number) {
  ...
  rect(rect: Rect, fill?: string | false, stroke?: string, width?: number) {
    const ctx = this.#ctx
  ...
  circle(circle: Circle, fill?: string | false, stroke?: string, width?: number) {
    const ctx = this.#ctx
  ...
  line(line: Line, a?: number | string, b?: number): void {
    const ctx = this.#ctx
  ...
  point(point: Point, fill?: string, width: number = 5) {
    const ctx = this.#ctx
